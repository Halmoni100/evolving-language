...
def format_name(envname, use_copier, maxcycle, n_agents, copier_lb=None):
    if use_copier:
        name = '{}_copier#2lb{}_maxcycle{}_{}agent'.format(envname, copier_lb, maxcycle, n_agents)
    else:
...
def main(NUM_EPISODES: int,
    ...
    if not continue_from_checkpoint:  ## INITIALIZE EVERYTHING
        ...
        if use_copier:
            ...
            for i in range(NUM_AGENTS):
                copier = Copier(env.num_agents-1, copier_ep_lookback, obs_dim=OBS_DIM, num_actions=NUM_ACTIONS)
                copier_list.append(copier)
        print("Copiers initialized!")
        ...
        copier_list = package['copiers']
        copier_nn_paths = package['copier_nn_paths']
        ...
        for i in range(len(agent_paths)):
            ...
            agent_list.append(agent_dqn)
        if use_copier:
            for i in range(len(copier_list)):
                copier_list[i].model = load_model(copier_nn_paths[i])
    ...
    for ep_i in eps_to_train: 
        ...
        # [COPIER] TRAIN COPIERS AT THE START OF EACH EPISODE
        if use_copier and ep_count > eps_until_learn_copier:
            for i in range(NUM_AGENTS):
                copier_list[i].train(epochs=copier_train_epochs)
        ...
        while not all(done_n): 
            ...
            for agent_i in range(env.num_agents):
                ...
                self_copier = copier_list[agent_i]

                # [COPIER] GET COPIER PREDICTION + EMBED
                if use_copier:
                    if ep_count > eps_until_learn_copier:
                        copier_prediction = self_copier.predict(obs_i)
                    else:
                        copier_prediction = 0
                ...
                # [COPIER] STORE OBS_I AND ACTION_I INTO COPIER BUFFER IN ALL OTHER COPIERS
                if use_copier and ep_count > eps_until_learn_copier:
                    for j in range(NUM_AGENTS):
                        if j != agent_i:
                            copier_list[j].store_obs_action(obs_i, action_i)
                ...
                if use_copier:
                    if ep_count > eps_until_learn_copier:
                        copier_prediction = self_copier.predict(new_obs_i)
                    else:
                        copier_prediction = 0
                    copier_prediction = ACTION2EMBEDDING[copier_prediction]
    ...
    if use_copier:
        copier_nn_paths = [os.path.join(version_save_dir,'copier_nn{}'.format(i)) for i in range(NUM_AGENTS)]
        for i in range(NUM_AGENTS):
            copier_list[i].model.save(copier_nn_paths[i])
            copier_list[i].model = None
            copiers_to_save = copier_list
    else:
        copiers_to_save = None
        copier_nn_paths = None
    ...
    save_package = {
        ...
        "copiers": copiers_to_save,
        "copier_nn_paths": copier_nn_paths,
